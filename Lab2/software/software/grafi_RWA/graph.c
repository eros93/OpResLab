...
void
free_PATH_ITEM (PITEM p)	/* Inserisce nella lista
...
void
make_empty_PATH_ITEM (PLINK plink)
{
  PITEM p;
...
void
free_linklist (PLINKLIST p)	/* Inserisce l'elemento p nella lista free */
{
  *p = NullLinkList;
...
void
make_empty_linklist (PGRAPH g, NODEID i)
{
  PLINKLIST p;
...
void
free_LinksOut (PLINK p)		/* Inserisce l'elemento p nella lista free */
{
  *p = NullLink;
...
void
make_empty_Links (PGRAPH g, NODEID i)
{
  PLINK p;
...
PLINKLIST find_linklist (PGRAPH g, PLINK p_link)
{
  ...
  if (plinklist == NULL)
    {
      perror
	("remove_link: link non trovato sulla lista dei link entranti!!\n");
      exit (1);
    }
  return (plinklist);
}
...
void
remove_linklist (PGRAPH g, PLINK p_link)
{
  ...
  if (plinklist != get_linklist (g, link_to (p_link)))	/* elemento non di testa */
    {
      ...
      if (linklist_next (plinklist) != NULL)	/* Non è l'ultimo */
...
void
remove_link (PGRAPH g, NODEID node, PLINK p_link)
{
  remove_linklist (g, p_link);

  /* rimuovo il link dalla lista dei link uscenti */

  if (first_link_out (g, node) != p_link)
    {				/* elemento non di testa */
      next_link (pred_link (p_link)) = next_link (p_link);
      if (next_link (p_link) != NULL)	/* Non è l'ultimo */
...
void
remove_item (PGRAPH g, PLINK p, PITEM p_item)
{
  if (first_path_item (p) != p_item)
    {				/* non è un puntatore di testa */
      next_item (pred_item (p_item)) = next_item (p_item);
      if (next_item (p_item) != NULL)	/* Non è l'ultimo */
  ...
  if (iter_start == 0)
    {
	  ...
	  for (j = 0; j < num_nodes; j++)
	    {
	      if ((i != j)
		  && (find_link (g, first_link_out (g, i), i, j) == NULL))
...
void
init_graph (PGRAPH g1, int num_nodes)
{
  ...
  for (i = 0; i < max_num_nodes; i++)
    {
      ...
      if ((node_graph (g1, i).matrix_path =
	   (PLINK **) malloc ((max_num_nodes) * sizeof (PLINK *))) == NULL)	/* 1 indice: destinazione */
	printf ("init_graph: Errore allocazione memoria\n");
      for (j = 0; j < (max_num_nodes); j++)
	{
	  if (
	      (node_graph (g1, i).matrix_path[j] =
	       (PLINK *) malloc ((max_num_nodes) * sizeof (PLINK))) == NULL)	/* 1 indice: destinazione */
	    printf ("init_graph: Errore allocazione memoria\n");
      ...
      if (
	  (node_graph (g1, i).incr_matrix_path =
	   (PLINK **) malloc ((max_num_nodes) * sizeof (PLINK *))) == NULL)	/* 1 indice: destinazione */
	printf ("init_graph: Errore allocazione memoria\n");
      for (j = 0; j < (max_num_nodes); j++)
	{
	  if (
	      (node_graph (g1, i).incr_matrix_path[j] =
	       (PLINK *) malloc ((max_num_nodes) * sizeof (PLINK))) == NULL)	/* 1 indice: destinazione */
	    printf ("init_graph: Errore allocazione memoria\n");
	  for (k = 0; k < max_num_nodes; k++)
...
/* Copia il grafo g1 in g2. Non copio la lista delle connessioni che si
   appoggiano sui vari link perché non mi servirà */
void
clone_graph (PGRAPH g1, PGRAPH g2)
{
...
// PATH_ITEM p_item;
  ...
  for (s = 0; s < num_nodes; s++)
	...
	{
	  link1 =
	    add_link (g2, link_from (link), link_to (link),
		      link_capacity (link));
